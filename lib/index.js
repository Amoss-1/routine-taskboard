...
/** Business health ledger (pipeline_status.json) written by the scheduled jobs. */
const PIPELINE_FILE = process.env.RTB_PIPELINE_FILE || ''
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (rest === 'pipeline' && req.method === 'GET') {
            if (PIPELINE_FILE === '') return sendJson(res, 200, { ok: false, error: 'pipeline file not configured (set RTB_PIPELINE_FILE env)' })
            try {
              const parsed = JSON.parse(readFileSync(PIPELINE_FILE, 'utf8'))
