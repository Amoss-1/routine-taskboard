...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var STYLE_TEXT = [
      ...
      ".rtb-row[data-health='paused'] { border-left: 3px solid #e5a000; }",
      ".rtb-row[data-health='paused'] .rtb-nm { color: #e5a000; }",
      ".rtb-row[data-health='fail'] { border-left: 3px solid #e5484d; }",
      ".rtb-row[data-health='fail'] .rtb-nm { color: #e5484d; }",
      ".rtb-badge { flex: none; font-size: 10px; line-height: 1; padding: 2px 6px; border-radius: 999px;",
      "  border: 1px solid var(--dsw-border, rgba(128,128,128,.3)); color: var(--dsw-alias-label-secondary, var(--dsw-text-secondary, gray)); }",
      ".rtb-badge[data-b='ok'] { color: #30a46c; border-color: rgba(48,164,108,.45); }",
      ".rtb-badge[data-b='paused'] { color: #e5a000; border-color: rgba(229,160,0,.45); }",
      ".rtb-badge[data-b='fail'] { color: #e5484d; border-color: rgba(229,72,77,.45); }",
      ".rtb-badge[data-b='none'] { opacity: .65; }",
      ...
      ".rtb-script { color: var(--dsw-alias-label-primary, var(--dsw-text, inherit)); overflow: hidden;",
    ...
    function makeController() {
      ...
      var that = {
        ...
        refresh: function () {
          // Fully self-contained: our own fixture registry + pipeline status.
          // No dependency on the official dsh-taskboard plugin (removed 0.1.9).
          return loadConfig().then(function () {
            return Promise.all([
              fetch(PREFIX + "/fixtures").then(function (r) { return r.json() }).catch(function () { return null }),
              fetch(PREFIX + "/pipeline").then(function (r) { return r.json() }).catch(function () { return null })
            ]).then(function (res) {
              var fixJ = res[0], pipeJ = res[1];
              if (fixJ && fixJ.ok && Array.isArray(fixJ.fixtures)) {
                var health = (pipeJ && pipeJ.ok && pipeJ.tasks) ? pipeJ.tasks : {};
                var fx = fixJ.fixtures.map(function (f) {
                  var rec = health[f.healthKey];
                  var st = rec ? rec.status : undefined;
                  f.health = st === "ok" ? "ok" : (st === "paused" || st === "disabled" || st === "disable") ? "paused" : st ? "fail" : undefined;
                  if (rec && rec.note) {
                    f.output = f.output || extractOutput(f.description || "", rec.note);
                    f.input = f.input || extractInput(f.description || "", rec.note);
                  }
                  var io = ioFor(f);
                  if (io) {
                ...
                that.setState({ fixtures: [], loaded: true, error: (fixJ && fixJ.error) || "fixtures unavailable" });
              }
          ...
          fx.map(function (f) {
            ...
            return react_jsx_runtime.jsx("div", { className: "rtb-grid rtb-row", "data-health": healthCls, children: [
              react_jsx_runtime.jsx("span", { className: "rtb-nm-wrap", children: [
                ...
                react_jsx_runtime.jsx("span", { className: "rtb-badge", "data-b": f.health || "none", title: (f.health === "ok" ? "运行正常" : f.health === "paused" ? "已停用" : f.health ? "异常" : "未知"), children: (f.health === "ok" ? "运行中" : f.health === "paused" ? "已停用" : f.health ? "异常" : "—") }),
                f.taskName && f.health !== "paused" ? react_jsx_runtime.jsx("button", { type: "button", className: "rtb-run", title: "立即启动计划任务", onClick: function () { runTask(f) }, children: "▶ 启动" }) : null,
                f.taskName && f.health !== "paused" ? react_jsx_runtime.jsx("button", { type: "button", className: "rtb-run", title: "停用此计划任务", onClick: function () { setTaskState(f, "disable") }, children: "⏸ 停用" }) : null,
                f.taskName && f.health === "paused" ? react_jsx_runtime.jsx("button", { type: "button", className: "rtb-run", title: "重新上线此计划任务", onClick: function () { setTaskState(f, "enable") }, children: "↻ 重新上线" }) : null
              ] }),
